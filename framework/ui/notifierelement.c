    ...
    void notifierelement_queuemessage( element_t* element , mtstr_t* message , input_t* input )
    {
        ...
        mtvec_adddata( data->messagequeue , message );
    ...
    void notifierelement_next( element_t* element , input_t* input )
    {
        ...
        vectoranimator_init( &data->animator , vector2_init(0.0, -data->messageelement->height ) , vector2_init(0.0, 0.0) , input->ticks , input->ticks + 60 , kAnimationTypeEaseOutExp );
			...
			case kInputTypeTimer    :
            {
                ...
                else if ( data->state == kNotifierStateIdle )
                {
                    if ( data->timer++ == 120 )
                    {
                        ...
                        vectoranimator_init( &data->animator , data->animator.actual , vector2_init(0.0 , -data->messageelement->height) , input->ticks , input->ticks + 60 , kAnimationTypeEaseInExp );
                        vectoranimator_step( &data->animator , input->ticks );
                    }
                }
                else if ( data->state == kNotifierStateDisappear )
                {
                    if ( data->animator.running == 1 )
                    {
                        ...
                        {
                            mtvec_removedataatindex( data->messagequeue , 0 );
