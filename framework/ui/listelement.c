    ...
    */
        ...
        float       scrollerwidth )
    {
        ...
        data->translation = vector2_init( 0.0, 0.0 );
        data->scrolltranslation = vector2_init( 0.0, 0.0 );
    ...
    char listelement_reset( element_t* element )
    {
        ...
        list->bounce = 0;
        list->momentum = 0.0;
        list->translation = vector2_init( 0.0 , 0.0 );
    ...
    {
        listdata_t* list = element->data;
        mtvec_adddata( list->headqueue , item );
    ...
    {
        listdata_t* list = element->data;
        mtvec_adddata( list->tailqueue , item );
    ...
    void listelement_do_scroll( element_t* element , input_t* input , float y )
    {
        ...
        while ( list->list->needshead == 1 )
        {
            ...
            if ( list->headqueue->length > 0 )
            {
                ...
                {
                    mtvec_adddataatindex( list->items , newhead , 0 );
                    mtvec_removedataatindex( list->headqueue , 0 );
        ...
        while ( list->list->removehead == 1 && list->items > 0 )
        {
            ...
            mtvec_adddataatindex( list->headqueue , head , 0 );
            mtvec_removedataatindex( list->items , 0 );
            element_removesubelement( element , head );
            
            list_removehead( list->list , head->height );
            if ( list->headqueue->length > list->buffersize ) mtvec_removedataatindex( list->headqueue , list->headqueue->length - 1 );
        ...
        while ( list->list->needstail == 1 )
        {
            ...
            if ( list->tailqueue->length > 0 )
            {
                ...
                {
                    mtvec_adddata( list->items , newtail );
                    mtvec_removedataatindex( list->tailqueue , 0 );
        ...
        while ( list->list->removetail == 1 && list->items > 0 )
        {
            ...
            mtvec_adddataatindex( list->tailqueue , tail , 0 );
            mtvec_removedataatindex( list->items , list->items->length - 1 );
            element_removesubelement( element , tail );
            
            list_removetail( list->list , tail->height );
            if ( list->tailqueue->length > list->buffersize ) mtvec_removedataatindex( list->tailqueue , list->tailqueue->length - 1 );
