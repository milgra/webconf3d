...
//            "       gl_FragColor = texture2D( texture_video, texcoordv , 0.0 );\n"
    ...
    uirenderer_t* uirenderer_alloc( float width , float height )
	{
        ...
        if ( max_size > 8192 ) max_size = 8192;
	...
	char uirenderer_addelement( uirenderer_t* renderer , element_t* element )
	{
        ...
        if ( element->texture.tiled == 1 )
        {
            if ( element->bitmap != NULL )
            {
                     ...
                     element->texture.coordstamp.tv_usec != renderer->texreset.tv_usec )
                {
                    ...
                    element->texture.ulc = vector2_init( ulx , uly );
                    element->texture.urc = vector2_init( urx , ury );
                    element->texture.llc = vector2_init( llx , lly );
                    element->texture.lrc = vector2_init( lrx , lry );
		...
		floatbuffer_addvector2( buffer , vector2_init( element->finalx , element->finaly ) );
		floatbuffer_addvector2( buffer , element->texture.ulc );
		floatbuffer_addvector2( buffer , vector2_init( element->finalx , element->finaly + element->height ) );
		floatbuffer_addvector2( buffer , element->texture.llc );
		floatbuffer_addvector2( buffer , vector2_init( element->finalx + element->width , element->finaly + element->height ) );
		...
		floatbuffer_addvector2( buffer , vector2_init( element->finalx + element->width , element->finaly + element->height ) );
		floatbuffer_addvector2( buffer , element->texture.lrc );
		floatbuffer_addvector2( buffer , vector2_init( element->finalx + element->width , element->finaly ) );
...
//            renderer->uploadsize += size;
		...
		for ( int index = 0 ; index < renderer->groupcount + 1; index++ )
		{
			...
			vector2_t* translation = group->translation;
			...
			if ( translation != NULL ) projection.matrix = matrix4_translate( renderer->perspective_matrix , translation->x , translation->y , 0.0 );
